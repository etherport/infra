...
4. **Ceph MTU/path** (see §2 caveat) — ✅ **RESOLVED: moot for M52.** Storage nodes connect at 10G to Switch Rack 10G (USL8A, 7×10G ports up); intra-VLAN-210 Ceph replication (MTU 9000) is L2 on that switch and **never traverses an L3 ACL**. Our storage-tier BLOCK rules live on the US624P L3 engine and only affect *inter-VLAN routed* traffic, which leaves the initiating host at 1500 MTU. So jumbo state cannot interact with these ACLs.
   - **Separate finding — RESOLVED 2026-05-29 (not a problem):** the device-level `jumboframe_enabled: False` is misleading. Per-port inspection shows all 10G storage ports (USL8A 1-8, US624P 25-26) have **`jumbo: true`** at the port level — which is what actually carries the Ceph/vSAN 9000-MTU frames. Ceph + CNPG + all RBD PVCs healthy throughout. The global device flag doesn't reflect per-port config. No action needed.
