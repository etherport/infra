...
- **Source:** zero-trust assessment 2026-06-17. velero/rclone/CNPG-barman/ai-advisor auth to AWS with **long-lived static IAM keys in K8s Secrets** — same standing-credential class H29 killed in CI / M71 targets on the mini. **Diagnosis 2026-06-24:** ALL ~13 AWS-cred secrets across 8 namespaces carry the **SAME shared key** (`AKIA4C5DM33X…`, the H29 "never delete, rotate only" homelab key). **Do:** self-hosted IRSA → pods assume per-workload least-priv roles for short-lived creds, no static keys in etcd. Extends [[H29]]/[[M71]]. Full design + procedure: [`docs/runbooks/irsa-workload-identity.md`](../runbooks/irsa-workload-identity.md). **Effort:** M–L.
  ...
  - **⏳ Phase 3 (DISRUPTIVE — gated):** flip kube-apiserver `--service-account-issuer` to the bucket URL (primary) keeping `kubernetes.default.svc.cluster.local` (secondary, so existing tokens still validate). Rolling static-pod restart cp2→cp3→**cp1 LAST** (no HA API VIP — cp1 `10.10.201.50` IS the endpoint + devbox kubectl target; expect a kubectl blip). Per-CP validate (fresh `kubectl create token --audience=sts.amazonaws.com` carries new `iss`). Persist in kubespray inventory (dual-issuer extraArgs — confirm v1beta4 list mechanism). Procedure + rollback in the runbook.
  - **⏳ Phase 4:** deploy `amazon-eks-pod-identity-webhook`; per-workload annotate SA with `role_arns` + drop static-key secret (velero BSL credential; CNPG `inheritFromIAMRole: true`; rclone `env_auth=true`; ai-advisor/cloudwatch SDK chain) → verify → delete secrets → **rotate** (not delete) the shared key. NB **etcd-backup is host-level (CP systemd), NOT an IRSA target** → stays under [[M71]].
