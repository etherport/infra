...
- **Phase 3 TODO:** rclone Garage→S3 weekly DR CronJob + S3 Deep-Archive lifecycle (`Garage(NAS) → rclone → s3://…/dr/ → Glacier Deep Archive`) + a Garage-down alert. Until then S3 has no fresh offsite copy of NEW backups (old S3 repo is the read-only fallback through its 30-day TTL). **Effort: S (Phase 3 only). Tier: M.**
