...
- **🔑 Disk constraint (the crux):** osxphotos reads a **local** Photos library, but the mini has only **~11 GB free** (228 GB disk, 194 GB used) — nowhere near a full or even optimize-storage library. **Plan: host the Photos library in an APFS sparse disk image on the NAS (SMB), mounted on the mini** (macOS sees a local APFS volume; bits live on the NAS) → "Download Originals" into it → `osxphotos export --update` (XMP sidecars) → individual files+sidecars → S3. The sparsebundle (working library, one huge file) is **excluded** from S3; only the exported files are synced. **Caveat:** sparsebundle-over-network has corruption/perf risk if the link drops mid-write (Time-Machine model; acceptable for a mostly-append backup-source lib — and the lib is just a cache of iCloud, so corruption = rebuild not data-loss). **Robust alt:** external APFS SSD on the mini (no network risk; needs hardware).
- **Plumbing settled 2026-06-18:** share = **`Personal-Drive`** on `sequoia` (SMB), mounted at **`/Volumes/Personal-Drive`** (the existing Finder mount — all NAS shares auto-mount under `/Volumes`; mounting the same share at a custom path fails EEXIST). Library sparsebundle → `/Volumes/Personal-Drive/Photos/PhotosLibrary.sparsebundle`; export → `/Volumes/Personal-Drive/Photos/export/`. **Mount persistence DONE:** `infra/macos/mini/` `net.wind.mount-nas` LaunchAgent ensures `/Volumes/Personal-Drive` at login via `open smb://` (idempotent; login keychain), installed + verified (last exit 0). Export goes to a **dedicated `photos` S3-sync share → its own bucket in Glacier Instant Retrieval** (owner's pick — NOT the existing shares' Deep-Archive `archive` bucket), so a specific photo is instantly retrievable.
- **Remaining build:** sparsebundle-create helper + `osxphotos export` script (preflight: `mount-nas.sh` → `hdiutil attach` sparsebundle → export) + `launchd` timer; TF for the photos bucket + scoped IAM; new `photos` aws-s3-sync share. Then owner: Photos sign-in → set that library → Download Originals → first 2FA.
