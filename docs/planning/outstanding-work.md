...
- ⏳ **Supervised applies (gated):** (1) `terraform apply` the s3 stack (creates bucket + IAM) — CI workflow_dispatch or headless; (2) `terraform output -raw etcd_backup_access_key_id`/`_secret_access_key` → SOPS-encrypt into `infra/ansible/playbooks/secrets/etcd-backup.sops.yaml` (keys `etcd_backup_aws_access_key_id`, `etcd_backup_aws_secret_access_key`); (3) run `etcd-backup.yml --limit kube_control_plane`. The PrometheusRule + pushgateway pin ship via Flux automatically. **Effort:** S remaining (applies).
