...
### 🟡 H37. Proxmox host firewall (host mgmt plane) — Stage 1 LIVE (permissive+observing); Stage 2 pending
...
- ⏳ **Stage 2 (enforce):** once observation confirms coverage, flip `local.input_policy` ACCEPT→DROP (`main.tf`) + revert rule `log` to `nolog`, re-apply. **Break-glass = IPMI/console — confirm before flip.** Reversible. **Effort:** S remaining (observe + flip).
