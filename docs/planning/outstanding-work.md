...
### 🟡 H37. Proxmox host firewall (host mgmt plane) — Stage 1 authored, BLOCKED on PVE privilege grant
...
- 🚧 **BLOCKED applying Stage 1:** the `graham@pam!terraform` token has broad VM/datastore/SDN privs but **NOT `Sys.Modify`** → PVE returns `403 Permission check failed (/, Sys.Modify)` for all datacenter/node firewall writes. **Owner action (PVE root) needed:** grant `Sys.Modify` on `/` to the token — recommended via a dedicated least-priv role, e.g. `pveum role add TerraformFirewall -privs "Sys.Audit Sys.Modify"` + `pveum acl modify / -roles TerraformFirewall -tokens 'graham@pam!terraform'` (or use a *separate* fw-only token for stricter isolation). Then re-run the Stage-1 apply.
- ⏳ **Stage 2 (after Stage-1 observe):** flip `local.input_policy` ACCEPT→DROP once logs confirm TS/WG/mini/laptop sources hit `mgmt-admin`. **Break-glass = IPMI/console** (confirm before flip). **Effort:** S remaining (grant + apply + observe + flip).
