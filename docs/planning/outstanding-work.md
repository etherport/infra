...
- **🟡 Owner setup IN PROGRESS (2026-06-18):** new System Photo Library created inside the sparsebundle (`Photos Library (NAS).photoslibrary`); iCloud "Download Originals" **actively downloading** (sparsebundle growing; `cloudphotod.initialdownload` assertion live). Old `~/Pictures` library left untouched as fallback. **Remaining:** wait for download (days) → enable the `net.wind.photos-export` timer + seed first export (commands in README) → first 2FA.
