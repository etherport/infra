...
| 7 | **L21 GCP tidy-up** | `terraform-google` red since before OIDC — empty `GCP_SA_KEY` secret | **Finisher (do after M69).** Decide: populate `GCP_SA_KEY` if the GCP module is wanted, else remove the module + its workflow. Stops the one red CI check (also the lone failure on H30 pin PR #62). |
