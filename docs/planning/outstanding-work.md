...
### 🟡 H45. CVE patch batch: ⏳containerd 2.2.1→2.2.5 (window), ✅Cilium 1.18.11, ✅CNPG operator 1.24.1→1.30.0, ✅PG 16.4→16.14
...
### ⏳ L32. vpn-local Tailscale node offline 47d — re-enable tailscaled (NOT an IP-swap issue)
...
### 🟡 M128. Rename `vpn-local` → `vpn-fallback` — repo+PVE+guest DONE 2026-07-02; TS re-join pending (auth key)
...
### 🟡 M129. Enable local TS exit nodes — k8s-homelab-router APPROVED (owner, 2026-07-02); vpn-fallback blocked on L32 reinstall
