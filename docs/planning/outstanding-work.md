...
| 6 | **M69 Cloudflare provider v4→v5** | 🟡 in progress — v5 HCL rewrite validates on branch `cf-provider-v5-migration`; state import pending (windowed) | Plan in [`cloudflare-provider-v5-migration.md`](cloudflare-provider-v5-migration.md). Renovate #48 closed (auto-bump would break plan). personal-web does its (DNS-only) half first as proving ground. |
| 7 | **L21 GCP auth → WIF** | `terraform-google` red since inception — empty `GCP_SA_KEY`; **no state exists** (never applied) | **Decided 2026-06-13: make it live via GitHub→GCP Workload Identity Federation** (no static key, mirrors H29). Plan + bootstrap commands in [`gcp-oidc-wif-l21.md`](gcp-oidc-wif-l21.md). Step 1 (gcloud WIF bootstrap) is **owner-gated**; then agent wires the workflow + imports the 2 projects. Closes the lone red CI check (also the only failure on H30 pin PR #62). |
