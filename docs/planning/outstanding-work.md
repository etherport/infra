...
- **🟡 Resume RUNNING in LOCAL mode (2026-06-19 ~12:17):** the PhotoKit resume ran clean for ~4h49m (10.2k→30.7k files) then hit two residual issues: (1) the write-heavy `Backups` share *still* dropped once (mc_on=no improved it 30s→~5h but didn't fully eliminate it; wrapper self-healed: killed run cleanly rc=143, remounted, retried); (2) an SMB *reconnect* (transparent — mount survived) made **Photos.app quit** ("library moved/corrupt" dialog) which **wedged** osxphotos on `CoreData: XPC: failed after N attempts` (0 progress). **Root insight: all the Photos.app/dialog/XPC fragility comes from `--use-photokit`.** Since attempt 1 already downloaded the originals into the library, switched the resume to a **pure-LOCAL export** (`DOWNLOAD_MISSING` now opt-in in `photos-export-resume.sh`; default off = no PhotoKit, no Photos.app, no corruption dialogs, no XPC wedge — reads library files directly, reports any not-yet-local original as "missing"). Local run launched 12:17 (30,774 files). **Plan:** local pass completes + writes the DB → then one short `DOWNLOAD_MISSING=1` pass for the few genuinely-missing originals → `CLEANUP=1` reconcile.
- **Remaining (after resume completes):** verify the full ~14.3k exported → enable the `net.wind.photos-export` timer → confirm `s3-sync-backups` picks up `Graham/iCloud/Photos`. Once the export DB is healthy, a one-off `CLEANUP=1` run (or the nightly `photos-export.sh`, which keeps `--cleanup`) re-establishes deletion mirroring.
