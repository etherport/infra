...
- **Residual housekeeping:** the 06-19 nightly's stale PartiallyFailed/Failed backups will be superseded by tonight's clean run. The velero alert silence (`4fe8a806`) auto-expires 2026-06-19T19:22Z; between then and tonight's nightly the stale partials may fire ≤1 advisor email (once-per-day cap) — owner can extend/lift the silence as preferred. Two `m84-verify-*` on-demand backups left as fresh restore points (postgres labeled to the schedule; infra standalone).
