...
- **Found 2026-07-02** (owner tidy-up). `tailscale status`: `vpn-local` (100.73.247.54) **offline, last seen 47d ago**. Investigated: the **VM is UP** (28d uptime, SSH-able) but `systemctl is-active tailscaled` = **inactive** and it didn't come back on the last boot (tailscaled not enabled / failed). **Answer to "should TS have a non-swapping IP like WG?":** it already does — a node's `100.x` CGNAT IP is **stable/persistent** (unlike the WG floating VIP), so Tailscale IS the stable-address path; vpn-local just needs its daemon back. **Fix:** SSH `ubuntu@10.10.201.15` → `systemctl enable --now tailscaled` → if the node key expired, `tailscale up` to re-auth → **disable key expiry** for it in the TS admin console (and consider tagging it `tagged-devices` so it's ACL-stable like cue-*/router). Quick (agent-doable next pass). **Effort: S.**
