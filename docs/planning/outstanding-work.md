...
### ⏳ L33. Re-evaluate the WireGuard HA-failover design (still the best approach?)
...
### ⏳ L34. Tailscale ACL / device-auth model — why did "abacus" work without an explicit allowlist add?
...
### 🟡 M123. K8s platform upgrade train — ✅1.34.3 patch DONE 2026-07-02 (with H45b); ⏳1.35 minor via kubespray v2.31 before Oct
...
### ⏳ M125. Migrate unifi TF provider off the archived paultyng/unifi — NOT a simple drop-in (2026-07-02 attempt reverted)
...
- **Re-attempt options:** (a) pin the ubiquiti-community fork to a version that still matches paultyng 0.41's resource names (check its early tags), OR (b) accept 0.54's renamed resources and `terraform state mv` each resource to the new type + update the .tf, OR (c) try the **akerl/unifi** fork. Investigate which fork/version keeps `unifi_network`/`unifi_port_forward`/`unifi_user` before swapping. **Effort: M (was mis-scoped as S).** Original context:
