...
- **Next:** (1) **wire `platform/kubernetes/networkpolicies/` into `clusters/wind`** (CNPs select only `netpol.wind/enforced=true` namespaces — none labeled yet, so applying is a no-op until labels are added). (2) **Label a namespace** (start `postgres`) → AUDIT verdicts flow. (3) Observe ≥1–2 wks via `hubble observe --verdict AUDIT` (cover CronJobs). (4) Write `1x-tier-*` allowlists from data. (5) Enforce per tier (postgres→cue→dns→traefik→monitoring; exclude wireguard/kube-system/flux-system) by labeling + finally disabling global audit.
...
- ⏳ **(a) Clean up `failed` helm release `cilium` (rev 15)** — DaemonSet healthy 8/8 but helm's stored manifest now drifts from the live ConfigMap (patched audit=true). A `helm rollback cilium <good-rev>`/`helm upgrade` fixes it but rolls Cilium → **supervised** (auto-mode classifier blocks helm here; needs owner to run it or grant a Bash permission rule). **Effort:** S.
