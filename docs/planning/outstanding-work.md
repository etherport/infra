...
- ✅ **Watchdog set the working way (2026-06-20):** attached the device to all 8 VM configs via the **PVE API `qm set --watchdog model=i6300esb,action=reset`** (config-only, no reboot — `current=[model=i6300esb,action=reset]` on all 8). Guest daemon already deployed+enabled on all 8 (ansible `k8s-node-fixes.yml` check = `changed=0`).
- ⏳ **Remaining = ACTIVATION:** the device only goes live on a VM **cold start**, so the watchdog activates per-node on the next reboot (kured cycles nodes naturally — zero forced reboots needed) → `/dev/watchdog0` appears → the enabled daemon pets it. Optional: force one reboot to verify e2e (`/dev/watchdog0` + daemon active). NB the 5 nodes rebooted today did so BEFORE the device was attached, so they too activate on their NEXT reboot.
