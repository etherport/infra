...
- ⚠️ **OPEN (infra-agent): mini→Traefik-VIP observability outage.** Since today's firewall/netpol/H38 cleanup the mini can't reach `10.10.201.70:443`, so all mini metric pushes (Pushgateway) + log shipping (Loki) fail — **backups themselves unaffected** (NAS path fine; pushgateway/loki healthy in-cluster). Infra-agent prompt (route fix + dashboard/alert wiring for the 5 new metric series) handed to owner. See session-log 2026-06-23 cont.4.
