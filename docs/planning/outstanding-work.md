...
- **Next (revised):** enable audit mode the **surgical way** — patch `cilium-config` `policy-audit-mode: "true"` + `kubectl rollout restart ds/cilium` (NOT kubespray; works now `/opt/cni/bin` is root-owned) → verify runtime `Enabled` on all agents → wire `networkpolicies/` into `clusters/wind` → observe ≥1–2 wks via `hubble observe --verdict AUDIT` → write `1x-tier-*` allowlists from data → enforce per tier (postgres→cue→dns→traefik→monitoring; exclude wireguard/kube-system/flux-system).
- **Effort:** L (phased over ~2 weeks of observation). **Largest internal-segmentation gap.**

### ⏳ H35. Clean up `failed` helm release for cilium (rev 15) + modernize kubespray run tooling
- **Source:** 2026-06-15 Cilium incident. (a) `helm history cilium -n kube-system` shows rev 15 **`failed`** (the aborted rollback) though the DaemonSet is healthy 8/8 — a `helm rollback cilium <good-rev>` now succeeds (dir is root-owned) but rolls Cilium, so **supervised**. (b) `infra/kubespray/kubespray.sh`/`setup.sh` wrappers are **stale** (expect a venv + `inventory/wind/` inside the submodule that don't exist; real path = `~/.kubespray-venv` + `inventory/inventory.ini`) — modernize them and make them **auto-run `pre-flight.yml` after `cluster.yml`** so the cni-owner fix can't be forgotten. (c) Evaluate adding `DAC_OVERRIDE` to Cilium's `mount-cgroup` via kubespray cilium values so it tolerates a kube-owned `/opt/cni/bin` (removes the footgun at the source). **Effort:** S (a) / M (b,c).
