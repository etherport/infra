...
- **🟡 DOWNLOAD_MISSING pass RUNNING (2026-06-19 22:15):** `DOWNLOAD_MISSING=1 photos-export-resume.sh` (PhotoKit, Photos.app open + cooperative) fetching the ~11,538 not-local originals → exporting them; multi-hour, self-healing, local DB preserves progress across restarts (downloaded originals persist in "Download Originals" mode → convergent). **Then:** `CLEANUP=1` (with `--dry-run` preview first) to delete the ~17k+ orphan duplicates → one canonical copy per photo.
