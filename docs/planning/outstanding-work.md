...
### 🟡 H44. Authentik upgrade — 2024.12.3 → **2025.8.6 DONE (CVE-2026-25227 REMEDIATED)**; HIGH-risk hops 5–8 pending a window
...
- **⏳ Hops 5–8 (2025.10.4 → 2025.12.6 → 2026.2.4 → 2026.5.3) — PAUSED, need an operator window.** These are HIGH risk: **hop 5 (2025.10)** removes Redis entirely (must bundle the redis-Deployment + `AUTHENTIK_REDIS__*` removal) + flips the OIDC `email_verified` claim default true→false (verify Grafana/wiki/Open WebUI don't gate on it); **hop 6 (2025.12)** is the RBAC overhaul — **global group-name uniqueness is REQUIRED or the `0056_user_roles` migration FAILS** (can't verify from here without DB/API access), + open unresolved upstream issues (#21362/#20667) with SSO-down-until-DB-restore failure mode. **Pre-flight done:** our **blueprints are CLEAN** (no `parent`/`ak_groups`/direct user-perms → RBAC + ak_groups deprecation won't break them); PG is plaintext (2025.10 TLS1.3 req N/A); no removed broker vars present. **Do these in a window with:** a per-hop CNPG backup, DB group-uniqueness confirmed, and **browser-login verification** available (the agent can't test logins — VLAN-201/VIP + no browser). Final target 2026.5.3. Also decommission redis at hop 5. **Effort: M. Owner: agent + operator window.**
