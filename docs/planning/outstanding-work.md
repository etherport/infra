...
### ✅ M91. k8s-vms hardware watchdog — DONE 2026-08-04, all 8 nodes armed + reboot-durable
- ✅ **RESOLVED 2026-08-04.** All 8 k8s VMs now run the **i6300esb hardware watchdog fed by systemd** (`RuntimeWatchdogSec=60`, `ShutdownWatchdogSec=10min`) → a wedged node hard-resets in ~60s via the PVE `action=reset`. Verified on every node: module loaded, `/sys/class/watchdog/watchdog0/state=active`, `fuser /dev/watchdog0` = **PID 1 only**, and the module listed in `/etc/initramfs-tools/modules`. IaC = **`infra/ansible/playbooks/k8s-node-watchdog.yml`** (idempotent, `serial: 1`, self-verifying — it *asserts* rather than assumes).
- 🎯 **Why it mattered:** this is the automatic recovery for the [[cp1 kernel hung-task/fork deadlock]] (2026-07-24 + 2026-08-04), which previously needed a manual `qm reset` from the PVE host. Paired with [[H47]] (HA API VIP), a single wedged CP is no longer a cluster-wide outage.
- 🔑 **The premise was wrong all along** — the long-standing "`i6300esb.ko` is ABSENT from the node kernel" note was false. **`linux-modules-extra-$(uname -r)` was simply never installed**; the module ships there, in the standard Ubuntu repo. Installing it + `modprobe` made `/dev/watchdog0` appear immediately. ~7 reboots and 3 incidents were spent chasing an exotic explanation for a missing package.
- ⚠️ **Gotcha 1 — the module is DENY-LISTED by Ubuntu, so `/etc/modules-load.d/` silently does nothing.** `/lib/modprobe.d/blacklist_linux_<kernel>.conf` (shipped by `linux-modules`, **regenerated on every kernel bump**, so it can't be removed durably) contains `blacklist i6300esb`. `systemd-modules-load` honours it and skips the module — *"Module 'i6300esb' is deny-listed (by kmod)"*. Caught for real: cp3 + w4 came back from the 08-04 kured sweep with the PCI device present but **no module, no `/dev/watchdog0`, watchdog silently gone**. **Fix = load it from the initramfs** (`/etc/initramfs-tools/modules` + `update-initramfs -u`): initramfs-tools runs a bare `/sbin/modprobe $m` with **no `-b`**, so the deny-list doesn't apply, and it loads *before* PID 1 — which matters because systemd opens `/dev/watchdog0` once at manager start and never retries.
- ⚠️ **Gotcha 2 — two watchdog consumers must never share the device.** The legacy `watchdog` **daemon** (installed by the original M91 attempt) sits dormant while `/dev/watchdog0` is absent, then wakes the instant the module loads, runs *its* health checks, and reboots the node on a failed one — that is what rebooted **cp1 at 12:11 on 2026-08-04**. The playbook now **disables + masks it BEFORE the modprobe**; systemd is the sole owner (the `holder=1` assertion enforces this).
- ⚠️ **Gotcha 3 — the emulated PCI device only appears on a COLD start** (`qm stop` + `qm start`), never a warm `reboot`. The playbook is a deliberate no-op on nodes lacking it and tells you to cold-start them. All 8 have been cold-started.
- 🪤 **Verification trap worth remembering:** `lsinitramfs … | grep i6300esb` is **not** a valid durability check — Ubuntu's default `MODULES=most` bundles the `.ko` into every initrd regardless, so it passes even when the module will never be *loaded*. Assert on `/etc/initramfs-tools/modules` (which becomes the initrd's `/conf/modules`) instead.
- 📌 **Durability:** `/etc/initramfs-tools/modules` persists across kernel upgrades and `update-initramfs` re-runs automatically on kernel install, so this survives kured/kubespray kernel bumps — strictly better than the per-kernel `-extra` reinstall the old plan assumed. Re-run the playbook after any node rebuild.

### ✅ M91 (historical detail — the dead ends, kept for the lessons)
...
- ✅ **DONE 2026-08-06:** read-only PAT minted + SOPS-encrypted (`github-mirror-token`; plaintext shredded
  off the devbox), CronJob `suspend: false`, verified end-to-end — manual run bundled all 6 repos (cairn,
  cue, cue-certs, gs-brand, infra, personal-web) to the NAS and a **restore from a NAS bundle** reproduced
  infra's HEAD + all 23 branches; all 6 bundles `verify` VALID. Runs on k8s (not devbox — devbox isn't in
  the NAS export allowlist). **Gotcha found:** GitHub git-over-HTTPS needs **Basic** auth (token as
  password), not the `Bearer` header the REST API accepts — clone 401s + prompts for a username otherwise.
  Rotation procedure (PAT expiry) in the dir README.
