...
### ⏳ M92. devbox can't dispatch GH workflows (no API token) — needs a fine-grained PAT
- **Source:** owner 2026-06-20 ("get it to me" / mirror laptop+mini). The devbox pushes via SSH but has **no GitHub API access** (no `gh`, no `GH_TOKEN`, no `~/.config/gh`), so an agent here can't `workflow_dispatch` (drift sweeps, TF applies). The ARC-runner `github_pat_…` in `github-actions-runner/secret.sops.yaml` is runner-registration-scoped (no Actions:write) — **do not repurpose**.
- ⏳ **Do:** create a fine-grained PAT (repo `sparked-diamond/infra` only, **Actions: read/write** + Contents: read), add it to `homelab-ops.sops.yaml` as `github_dispatch_pat`; then agents dispatch via the REST API. Full setup + curl examples + ZT caveat (this grants CI-apply = mutate-all-infra) in `infra/devbox/README.md` (§ "GitHub workflow dispatch"). Until then, drift applies (incl. [[M91]]) are dispatched from the laptop/mini.

### 🟡 M91. proxmox/k8s-vms drift — hardware watchdog not yet attached (rolling CI apply needed)
- **Source:** 2026-06-20 drift review. `terraform plan` on proxmox/k8s-vms = **0 add, 8 change, 0 destroy**: all 8 VMs `watchdog.action: none → reset` + gpu1 `memory.floating: 8192 → 0`. The watchdog (i6300esb, action=reset, guest daemon pets it → auto-reset on hang) is a **deliberate, documented** safety feature (`main.tf` + ansible `k8s-node-fixes.yml` "Enable hardware watchdog") that was authored but the apply attaching the device was never run — so live VMs have no watchdog. Directly relevant to the GPU dcgm wedge (a hung node would self-recover). NOT manual drift.
- ⏳ **Resolve (CI, rolling):** attaching the watchdog device likely **restarts the VM**, so do NOT apply all 8 at once. Dispatch `terraform-proxmox-k8s-vms` → `action: apply` with the **`target`** input ONE node at a time (start a worker e.g. `proxmox_virtual_environment_vm.workers["k8s-w4"]`, confirm clean reboot + rejoin, then proceed; control-planes one-by-one last). gpu1's apply also clears the floating-memory diff. Until done, the new CI drift sweep ([[M90]]) will flag k8s-vms each day — expected.
