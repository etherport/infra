...
### 🟡 M137. Durable fix for velero S3 egress — local-primary MinIO on the NAS (3-2-1)
- **Why:** velero's Kopia repo-maintenance (`"rewriting contents from short packs"`) DOWNLOADS repo content from S3 to repack → the 2026-07 egress (453 GB/7d, forecast $75→$160; see [[velero-kopia-maintenance-s3-cost]]). On-prem cluster reads S3 over the internet = billed egress. Durable fix = primary Kopia repo LOCAL, S3 as batched DR only. **Decision 2026-07-08:** MinIO on the **NAS over NFS** — NOT Ceph-RBD (pool size=2, <800 GiB usable; a 400 GB repo = >50% of headroom) and Ceph-on-NFS isn't viable. NAS has 22 TB free, NFS-only (no iSCSI). **Phase 1 BUILT (staged):** `platform/kubernetes/minio/` — single-node MinIO (`/data`←`sequoia:/var/nfs/shared/VeleroBackup` inline NFS) + Service + SOPS root creds + bucket-init; wired **COMMENTED-OUT** in `clusters/wind/kustomization.yaml` (alert-safe until the share exists). **BLOCKED on operator:** create UniFi Drive share `VeleroBackup` (~1 TB); NFS export writable by uid/gid 1000. **Phase 2:** repoint velero BSL → MinIO (fresh repo), keep old S3 BSL read-only 30d. **Phase 3:** rclone MinIO→S3 DR + Deep-Archive lifecycle + MinIO-down alert. **Effort: M. Tier: M.**
...
### ⏳ M135. cloud-tag-drift over-fires — TF stacks not propagating `default_tags{ManagedBy=terraform}`
- **State 2026-07-05:** the daily email's `cloud-tag-drift` row is a persistent "down" — 228/292 AWS resources lack `ManagedBy=terraform`. Two populations: (a) ~75 are structurally **untaggable / AWS-predefined** (CodeDeployDefault configs `0000…0001`, `AppConfig.AllAtOnce` strategies, `FARGATE`/`FARGATE_SPOT` capacity providers, `VPCBlockPublicAccessOptions` account setting, auto-defined resolver associations, KMS aliases, S3 StorageLens default) → should be added to the `ALLOWLIST` in `.github/workflows/cloud-tag-drift.yml`; (b) the rest (IAM roles/policies/users/groups, EC2 subnets/VPCs/SGs/route-tables, KMS keys, Lambda, S3 buckets, CloudWatch alarms) are **IaC-managed but their stack never set `default_tags`** → real hygiene drift. **Fix:** add `default_tags{ManagedBy="terraform"}` to every provider block missing it + re-apply each stack, and allowlist population (a). Until then the row stays red (not an outage — a hygiene signal). **Effort: M. Tier: M.**
