...
### ⏳ H36. Flux reconciliation alerting (silent GitOps-wedge gap)
- **Source:** 2026-06-17 — a CNPG webhook cert inconsistency wedged the flux-system Kustomization (`Ready=False`) for ~hours, blocking ALL GitOps, and was found only by luck (mid-M62). **`gotk_reconcile_condition` metrics are NOT scraped** (no flux PodMonitor; Prometheus query returns 0) — so Flux failures are invisible.
- **Fix:** add a PodMonitor for the flux controllers (kustomize/source/helm/notification-controller, metrics port `http-prom`/8080) → verify `gotk_reconcile_condition` appears → add a `PrometheusRule` `FluxReconciliationFailed` (`gotk_reconcile_condition{type="Ready",status="False"} == 1 for 15m`, per kind/name). Closes "GitOps silently stuck, nobody knows." **Effort:** S–M.
...
### ⏳ L23. Clean up orphan `cnpg-manager` ServiceAccount/RBAC (post duplicate-operator removal)
- **Source:** 2026-06-16. A **duplicate CNPG operator** (`cnpg-controller-manager`, from a pre-Helm raw-manifest install, not in git) was fighting the Helm operator (`cnpg-cloudnative-pg`) for the same leader-election lease → operator restart loop (37+29 restarts; the AI advisor's "stuck pod" alert). **Resolved** by deleting the orphan deployment; Helm operator is now uncontested leader, restarts stopped, DBs healthy throughout (see session-log 2026-06-16). **Residual (harmless):** the orphan's `ServiceAccount/cnpg-manager` in `cnpg-system` (+ likely a cluster-scoped `cnpg-manager` ClusterRole/Binding granting operator perms) remain — unused (no pod references them) but worth deleting for hygiene. Verify nothing references `cnpg-manager` first, then delete. **Effort:** S. **Note:** the AI advisor correctly did NOT offer a remediation button — `cnpg-system` is on its hard denylist (advisory-only for critical stateful namespaces); expected behavior, documented in the session-log.
