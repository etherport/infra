...
### 🟡 M135. cloud-tag-drift → green: re-apply AWS stacks to propagate `default_tags` + delete default VPCs
- **2026-07-08 progress:** detector reworked (`cloud-tag-drift.yml`) to exclude AWS-managed/untaggable resources — expanded the type allowlist (KMS aliases, associations, CodeDeploy/AppConfig/FARGATE defaults, account settings, ResourceExplorer/AccessAnalyzer/StorageLens, Cassandra system keyspaces) + arn-based exclusion of service-linked IAM roles & AWS-managed policies (SELECT now pulls arn/resourceName). **Count 228 → 118.** Confirmed via claude-admin: 0 service-linked roles (all 33 IAM roles are customer/TF), and 0 of 31 customer policies carry the tag — so the 118 are TF resources that PREDATE `default_tags` (all stacks now set `ManagedBy=terraform`, but the resources weren't re-applied). **To reach 0:** (1) re-apply each AWS TF stack — terraform will add the marker to existing IAM/EC2/KMS/Lambda/S3/CloudWatch resources (⚠️ review each plan = tags-only before apply; some stacks may carry other drift); (2) `iam-policies` has no provider (JSON-only, attached via groups) — those group-policy attachments may need their owning stack re-applied; (3) **delete the 2 default VPCs** (us-west-2 + us-east-1) — unused (TF uses its own VPC), removes ~18 untaggable default-VPC resources from Config, and is a security best practice. **Effort: M (bounded, mechanical). Tier: M.**
