...
### 📋 M103. `arc` — native iCloud Backup Agent (replace the bash suite)
...
- **Design doc:** [`backup-agent-design.md`](backup-agent-design.md) (DRAFT, awaiting review). Metrics kept drop-in compatible with the current dashboard/alerts; **migration is incremental + reversible** (run in parallel to a staging dest, diff per category, cut over one at a time, retire each bash agent only after parity). Lives at `tools/arc/` (Swift package) in the monorepo. **Effort:** L. **Status:** design drafted; build in 6 milestones (skeleton→mirror→native sources→photos→notifiers→daemon/CI/cutover).
