...
- ✅ **Fixed (`2f85c28`):** added a self-hosted `plan-internal` matrix job (cloudflare, unifi, proxmox firewall/sdn/standalone-vms/k8s-vms) on the `[self-hosted, lifecycle]` runner with the per-stack GH-secret creds (PVE token / CF token / UDM creds), feeding the same `report` job → one `tf-drift` issue covers everything. Corrections remain per-stack `workflow_dispatch` apply (OIDC) — never from a workstation.
