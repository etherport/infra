...
- ⏳ **To actually finish (deliberate follow-up, not urgent):** find/build the `i6300esb` module for this kernel (which package ships it? is the node on a Proxmox/HWE kernel variant that dropped it?), OR switch the daemon to `softdog` (guest-software watchdog — catches userspace/daemon hangs but NOT a hung kernel, so weaker), OR drop the watchdog entirely (it's been inert since inception, so no regression). Drift is already clean (ignore_changes); the attached device is harmless + ready for when the module exists.
- **Lesson (CLAUDE.md §5):** this consumed ~7 node reboots + 3 incidents chasing a feature that can't work without the kernel module — **verify the kernel module exists before attaching watchdog devices / rebooting.**
