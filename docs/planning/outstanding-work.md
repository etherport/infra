...
### 🟡 M84. Velero PVB "prepare timeout" under nightly load — node-agent dataPathConcurrency=1
- **Source:** 2026-06-19 nightly run. Beyond the H39 firewall fix (proven — most PVBs complete clean incl. a 6.8 GB one), a **subset of PVBs stall in `Prepared` and hit `timeout on preparing PVB`** → backups `PartiallyFailed` (06-19: critical-apps 1, infrastructure 4). **Root cause:** there is **no `node-agent-config` ConfigMap**, so velero node-agent uses the default **`dataPathConcurrency=1` per node** — when a backup has several volumes on one node (or batches overlap), PVBs serialize per node and the queued ones exceed the prepare timeout. NOT the firewall, NOT data corruption.
- **Fix (durable):** add a node-agent config raising `loadConcurrency.globalConfig` to ~2–3 and wire it to the node-agent (velero `nodeAgent` values / `--node-agent-configmap`), then `rollout restart ds/node-agent`. **Apply when no backup is in flight** (a node-agent restart kills in-flight PVBs). **Effort:** S. Verify with an on-demand multi-volume backup (e.g. infrastructure) completing with 0 prepare-timeouts.
...
- **🟡 First bulk export PAUSED at 71% — NAS SMB mount went unstable** (2026-06-19): full export ran cleanly to **10,201/14,343** (~3.5 h), then the `Backups` SMB mount dropped mid-run + kept dropping within ~30 s even idle (wedged server-side SMB session for that share; `Personal-Drive` fine, NAS 445 open + 0.4 ms ping). osxphotos **wedges** (doesn't exit) on the resulting PhotoKit XPC loss. **Data safe** (10.2k files on the NAS disk; SMB is just transport). Added **`infra/macos/mini/photos-export-resume.sh`** — self-healing wrapper (remount → ensure Photos up → `osxphotos export --update` resuming from the export DB, under a watchdog that kills+retries on mount-loss / zero-progress). **Resume (owner, VNC):** cancel the stuck NetAuthAgent SMB dialog → restart SMB / toggle the `Backups` share / reboot the UNAS → `mount-nas.sh` (confirm it holds) → `photos-export-resume.sh` (continues from 71%, no re-download). Full incident + lessons in [`session-log.md`](session-log.md) 2026-06-19.
- **Remaining (after resume):** finish the ~4k-photo tail → enable the `net.wind.photos-export` timer → confirm `s3-sync-backups` picks up `Graham/iCloud/Photos`.
- **Caveats:** Apple **2FA session expires** every ~weeks→months → periodic interactive re-auth via VNC. Sleep disabled (`SleepDisabled`/`sleep 0`). **SMB-over-load fragility** is now a known risk for the bulk pull → use the resume wrapper. **Effort:** M (tail + verify).
