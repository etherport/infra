...
### 📋 M87. cloudwatch-to-loki — cut CronJob over to the baked image (drop runtime pip)
- **Source:** 2026-06-20 overnight `BackoffLimitExceeded` (runtime `pip install` + `backoffLimit:0`). **Immediate fix shipped (`734fe3f`):** `backoffLimit 0→3` + `pip --retries 5 --timeout 30` (a transient blip now retries instead of paging). **Proper fix staged:** `platform/kubernetes/cloudwatch-to-loki/image/Dockerfile` + `.github/workflows/cloudwatch-to-loki-image.yml` bake `boto3`+`kubernetes>=30,<31` → `ghcr.io/sparked-diamond/cloudwatch-to-loki` (script stays in the ConfigMap).
- **Remaining (gated on a manual step):** (1) confirm the GH Actions build is green; (2) **one-time: flip the new ghcr package to Public** (`https://github.com/users/sparked-diamond/packages/container/cloudwatch-to-loki/settings`) — GITHUB_TOKEN can't do it for a user-owned package (same gotcha as cloudflare-ddns); (3) add an ImageRepository/ImagePolicy (mirror cloudflare-ddns) + switch the CronJob `image:` to the baked image and **delete the `pip install` line**. Verify a run, then pip is gone from the runtime path.

### 📋 M88. OneDrive → NAS backup (rclone-onedrive) — staged, awaiting OAuth token
- **Source:** owner 2026-06-20. Mirror of `rclone-gdrive` for a personal OneDrive (M365 personal sub = OneDrive **Personal**). New `platform/kubernetes/rclone-onedrive/` (`705e314`): nightly `rclone sync onedrive: → /backup/Graham/OneDrive/` (NAS Backups share → rides the existing NAS→S3 sync), 23:00 PT, `--tpslimit 10`, same Pushgateway metrics + failed/stale/errors/slow alerts.
- **Blocked:** rclone's OneDrive backend needs an **interactive browser OAuth** — can't run headless on devbox. Component is built but **NOT registered** in `clusters/wind/kustomization.yaml` and the secret is omitted (kustomization line commented), so nothing deploys half-built.
- **Activation:** user runs `rclone config` on a browser machine (create remote `onedrive`, personal MS account) → `rclone config show onedrive` → hand over the `[onedrive]` block → I create `04-secret.sops.yaml` (`sops --encrypt`), uncomment it in kustomization, register the dir in clusters/wind, reconcile, and verify a first sync. See `platform/kubernetes/rclone-onedrive/README.md`.
