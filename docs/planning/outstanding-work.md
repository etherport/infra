...
### 🟡 M135. cloud-tag-drift → green: the residual is bootstrap IAM (import-to-TF vs accept)
...
- **Residual = the ~51 `terraform-homelab` BOOTSTRAP IAM** (29 policies, 9 users, 8 roles, 5 groups) — hand-created per `iam-policies/README` (chicken-and-egg: TF needs an IAM identity to run). The detector *correctly* flags these as unmanaged. **Decision needed:** import them into TF (circular but codifies them) OR accept as intentional-manual (tag `ManagedBy=manual` + add a detector exclusion for that marker). Recommend the latter — cleaner than importing the bootstrap identity TF runs as.
- **Also surfaced (separate):** `external-monitoring` plan wants to CREATE `aws_sns_topic_subscription.email_backup[0]` — the BACKUP alert-email subscriber is missing (only a 1-resource add, not a tag change; skipped). Apply it to restore the 2nd alert-email channel, but it sends an SNS confirmation email that must be clicked. **Effort: S. Tier: M.**
