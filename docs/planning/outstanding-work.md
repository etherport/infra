...
- **Why:** velero's Kopia repo-maintenance (`"rewriting contents from short packs"`) DOWNLOADS repo content from S3 to repack → the 2026-07 egress ($75→$160; see [[velero-kopia-maintenance-s3-cost]]). Fix = primary Kopia repo LOCAL, S3 as batched DR only. **MinIO rejected NFS** ("insufficient drives online" — needs O_DIRECT) → switched to **Garage** (split storage: LMDB metadata on a 10Gi Ceph-RBD PVC + immutable data blocks on the NAS over NFS). **Phase 1 DONE + verified 2026-07-08:** `platform/kubernetes/garage/` deployed via Flux (runs as uid/gid 988 = share owner); bootstrapped (layout + imported velero S3 key `GK…` + `velero` bucket); 10MB S3 PUT → data block on the NAS ~90MB/s, small objects inline to RBD; survives restart (config persists in RBD metadata; bootstrap is a README runbook step, postStart hook dropped — it CrashLooped). S3 endpoint `garage.garage.svc:3900`, creds in `garage-velero-creds`. **Phase 2 (next):** repoint velero default BSL → Garage (fresh Kopia repo), keep old S3 BSL read-only 30d. **Phase 3:** rclone Garage→S3 weekly DR + Deep-Archive lifecycle + Garage-down alert. **Effort: M. Tier: M.**
