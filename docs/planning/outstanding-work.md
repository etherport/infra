...
- **Own repo** (standalone product): [sparked-diamond/cairn](https://github.com/sparked-diamond/cairn), cloned at `~/code/cairn`. Canonical design = `cairn/DESIGN.md` (infra's [`backup-agent-design.md`](backup-agent-design.md) is now a pointer). Metrics kept drop-in compatible with the current dashboard/alerts; **migration is incremental + reversible** (run in parallel to a staging dest, diff per category, cut over one at a time, retire each bash agent only after parity). **Mini = dev/build/test AND deploy** (owner installing full Xcode for `swift test` + code signing). **infra's role:** document `cairn`'s mini deployment (launchd/TCC/signing runbook) once built. **Effort:** L. **Status (2026-06-24):** repo scaffolded — compiling Swift skeleton (CairnKit lib + thin `cairn` CLI via swift-argument-parser, swift-testing, macOS CI). Build M1→M6 (skeleton→mirror→native sources→photos→notifiers→daemon/cutover).
