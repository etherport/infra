...
>
> **⚠️ Follow-on fix (the "re-verify routes.tf next-hops" step — initially missed):** the flip silently broke the **AWS-VPC path from every non-WG node**. The `routes.tf` AWS/WG routes went `UDM → 10.255.253.3 (4040) → switch → WG VIP .201.20`, relying on the switch's now-removed L3-on-201. Symptom: `ExternalHostDown`/`VPNGatewayDown`/`TechnitiumDNSDown` for `dns-aws`/`vpn-aws` from ~20:22, and `prometheus-1` (on non-WG node w2) showing `up=0` while `prometheus-0` (WG node w1, direct `wg0`) showed `up=1`. **Fix:** repointed the 3 UDM routes' next-hop `10.255.253.3 → 10.10.201.20` (UDM now reaches the WG VIP directly) and deleted the 3 dead `(L3)` switch routes (commit `3d43b1a` + state-cleanup `d3f321e`; the obsolete TF `syslog_port` group was dropped from state via a `removed{destroy=false}` block since Ansible owns it). Verified: w2 pod → AWS 0% loss, `prometheus-1` up=1, alerts cleared. **Lesson: a switch→UDM gateway move orphans any static route whose next-hop sits in the moved VLAN — re-point those to be directly reachable by the new gateway.**
