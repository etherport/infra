...
— **as of 2026-07-28 all 14 labeled tiers are enforcing** (audit-mode flipped back OFF after
the tiers 7-14 observation window closed, M147/M151; see `docs/planning/outstanding-work.md`
for history). Manifests: [`platform/kubernetes/networkpolicies/`](../../platform/kubernetes/networkpolicies/)
...
**Labeled tiers (14 live, all enforcing):** `postgres`, `cue`, `dns`, `traefik`, `monitoring`
(the 5 H3 target tiers, 2026-06), plus **`authentik`** (tier 6, M115 2026-07-01 — the SSO
IdP), plus 8 more labeled 2026-07-25 and confirmed enforcing since the 2026-07-28 audit-OFF
flip that closed their observation window: `flux-system`, `velero`, `backups`, `tailscale`,
`cert-manager`, `garage`, `home-automation`, `plex` (tiers 7-14, M147/M151 — note
`flux-system` IS now labeled, superseding the older "never label flux-system" guidance).
They all revert to audit-only together whenever the global switch is flipped ON to onboard
a future tier. Never label `kube-system`, `wireguard`, `metallb-system`.
...
> AWS replica `10.10.100.10` — on **tcp/5380** (admin API). Those targets are `reserved:world`, and
