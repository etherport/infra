# route53-ddns CronJob — Route53 → Cloudflare cutover
...
- `platform/kubernetes/route53-ddns/image/scripts/update-route53.sh` —
...
- `platform/kubernetes/route53-ddns/base/configmap.yaml` — `hosted_zones`
  replaced with `cf_zone_id`. AWS_* envs gone.
- `platform/kubernetes/route53-ddns/base/cronjob.yaml` — env vars
  switched to CF_ZONE_ID + envFrom now references
  `cloudflare-credentials` Secret.
- `platform/kubernetes/route53-ddns/base/cloudflare-credentials.sops.yaml.template`
...
on etherport.net) OR create a dedicated `route53-ddns` token for
...
sed -i '' 's|^  - 01-route53-secret.sops.yaml$|  - cloudflare-credentials.sops.yaml|' \
  platform/kubernetes/route53-ddns/base/kustomization.yaml
...
git commit -m "route53-ddns: cut over to Cloudflare + un-suspend"
git push
```
...
kubectl -n route53-ddns logs -l app=route53-ddns --tail=40
...
git rm platform/kubernetes/route53-ddns/base/01-route53-secret.sops.yaml \
       platform/kubernetes/route53-ddns/base/01-route53-secret.sops.yaml.template \
       platform/kubernetes/route53-ddns/base/secret-template.yaml

# 2. Delete the live AWS-cred Secret (Flux pruning would do this in ~10
#    min anyway; manual is faster):
kubectl -n route53-ddns delete secret route53-ddns-credentials
...
#    `.github/workflows/route53-ddns-image.yml` after a Dockerfile
...
## Why we kept the directory name "route53-ddns"

Rename across `platform/kubernetes/route53-ddns/`, the K8s namespace,
the image registry path, the CI workflow, the ServiceAccount, RBAC,
and PromQL `route53_ddns_*` metric names is a separate cleanup not
