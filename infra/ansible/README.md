...
| Standalone VMs | `technitium.yml`, `wireguard.yml`, `gh-runner.yml`, `asterisk-sbc.yml` | Per-service config for the 6 `proxmox/standalone-vms` TF VMs (dns-fallback, vpn-fallback, gh-runner, asterisk-sbc, devbox, step-ca — see that stack's [README](../terraform/proxmox/standalone-vms/README.md)) |
