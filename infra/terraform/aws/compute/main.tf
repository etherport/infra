...
# SSM Session Manager — break-glass console for the AWS instances. An EC2 has no PVE-console/IPMI
# equivalent, so once SSH goes cert-only + Tailscale-only, SSM is the recovery path if both fail
# (M110 hardening; "almost got caught by lack of SSM"). The ssm-agent snap ships preinstalled +
# enabled on the Ubuntu AMI; base.yml also ensures it's running on the AWS hosts.
