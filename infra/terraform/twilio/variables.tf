...
variable "sip_credential_list_sid" {
  description = <<-EOT
    ...
    registration to the Windtryst trunk. The list's username/password are
    NOT managed here — they live in 1Password + the UDM Talk config. This
    only links the existing list to the trunk by SID.
  EOT
  type        = string
