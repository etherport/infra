...
Alertmanager (severity=critical, continue:true)
   └─ webhook → am2ntfy bridge (:8080)  ── transforms AM JSON → ntfy publish
        └─ ntfy server (:8080)  ── topic "wind-critical", urgent priority
             └─ Tailscale LB svc (ntfy.tail48f596.ts.net)
...
| `05-tailscale-svc` | exposes ntfy to your Tailscale devices only (loadBalancerClass, tag:k8s) — **no public endpoint** |
...
1. After Flux applies, the Tailscale operator provisions the cert; find the URL:
   `kubectl -n tailscale logs deploy/operator | grep ntfy` → `http://ntfy.tail48f596.ts.net`.
2. Install the **ntfy** app, add server `http://ntfy.tail48f596.ts.net`, subscribe to
...
Tailnet-only (operator LoadBalancer, `loadBalancerClass: tailscale` + `tag:k8s`) is
the trust boundary —
